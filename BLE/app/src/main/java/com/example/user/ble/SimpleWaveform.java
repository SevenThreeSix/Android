...
public class SimpleWaveform extends View {
    ...
    public final static int MODE_ZERO_BOTTOM2 = 4;
    public int modeZero;
    ...
    public Paint textPencil = new Paint();
    ...
    private void getWidthLength() {
        ...
        post(new Runnable() {
            ...
            public void run() {
                ...
                Log.d("getWidthLength()长宽2","SimpleWaveform: w,h: " + width +" "+height);
    ...
    public void init() {
        ...
        Log.e("SimpleWaveform.java","init()");
        width = this.getWidth();
        height = this.getHeight();
        Log.d("init()长宽1","SimpleWaveform: w,h: " + width +" "+height);
        if (width > 0 && height > 0) {
            haveGotWidthHeight = true;
        }else{
        ...
        textPencil.setTextSize(50);
        textPencil.setColor(0xffffffff);
        firstPartNum = 0;
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        Log.d("SimpleWaveform.java", "normal view: onDraw()");

        if (clearScreenListener != null) {
            Log.d("SimpleWaveform.java", "clearScreenListener存在");
            canvas.drawColor(Color.BLUE, PorterDuff.Mode.CLEAR);
            clearScreenListener.clearScreen(canvas);
        } else {
            Log.d("SimpleWaveform.java", "画布颜色");
            canvas.drawColor(Color.BLUE, PorterDuff.Mode.CLEAR);//0x3F51B5
        }
        ...
        canvas.drawColor(0xff119999);//0x3F51B5 colorPrimary
        drawWaveList(canvas);
    ...
    private void drawWaveList(Canvas canvas) {
        ...
        for (int i = 0; i < barNum; i++) {
            ...
            if (modeHeight == MODE_HEIGHT_PERCENT) {
                if(modeZero == MODE_ZERO_BOTTOM2)
                    barPoints.amplitudePx = (barPoints.amplitude * (height-height/6) / 100);
                else
...
//                }
            ...
            switch (modeZero) {
                ...
                case MODE_ZERO_BOTTOM:
                    ...
                    break;
                case MODE_ZERO_BOTTOM2:
                    barPoints.amplitudePxTopCanvas = -barPoints.amplitudePxTop + height-height/6;
                    barPoints.amplitudePxBottomCanvas = -barPoints.amplitudePxBottom + height-height/6;
                    break;
            }
            ...
            if (showBar) {
                if(i%4 == 0) {
                    ...
                    this.barPoints[i * barUnitSize + 1] = barPoints.amplitudePxBottomCanvas + 10;
        ...
        if (showBar) {
            ...
            for(int i=0;i<barNum;i++){
                if((i+4)%8 == 0) {
                    try {
                        long timeTemp = simpleDateFormat.parse(timeStart).getTime()+10000*((i+4)/8-1);
                        Log.e("SimpleWaveform.java","timeTemp:"+timeTemp);
                        Date date =new Date(timeTemp);
                        String timeNext = simpleDateFormat.format(date);
                        canvas.drawText(timeNext, barPoints[i * peakUnitSize], (height - height/6) + 50, textPencil);
                    } catch (ParseException e) {
                        e.printStackTrace();
                    }
                    //canvas.drawText(timeStart, barPoints[i * peakUnitSize], (height - 100) + 50, textPencil);
                }
            }
        }
        if (showPeak) {
        ...
        if(showXAxis){
            ...
            canvas.drawLines(xAxisPoints, barPencilSecond);//xAxisPencil
            if(modeZero == MODE_ZERO_BOTTOM2) {

                for (int temp = 1; temp < 5; temp++) {//5等分
                    xAxisPoints[1] = xAxis_y - temp * height/6;
                    xAxisPoints[3] = xAxis_y - temp * height/6;
